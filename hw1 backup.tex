...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  \begin{parts}
    ...
    \begin{mysolution}{thegroupzpc}
       I claim that g is cyclic. Proof: Choose $g \in \Z_p^* such that g is relatively prime to (p-1)$. Then by FLT, we know that $g^{p-1} =1$ Now suppose that g is not a generator, i.e. that $\exists p_i|(p-1) such that g^{\frac{p-1}{p_j}}=1$. 
    \end{mysolution}
    ...
    \begin{mysolution}{thegroupzpd}
      We know that any generator $g \in \Z_p^*$ must satisfy $g^{p-1} = 1$. If g is a generator, then there must also be no integer $x <(p-1) such that g^x =1$. By Lagrange's Theorem, $|g|$ must divide (p-1). Take the prime factorization of $(p-1) = p_1 * p_2 *...*p_k$.  Supposing such an g exists and $|g|=q$, then there is some subset of the prime factors (call it S) such that $g \prod_{i \in S} p_i = (p-1)$. Now let $p_j \in S$ be any prime factor. Since $g^{p-1} =1$, we can write $(g^q)^{\prod_{i \in S} p_i} =( (g^{q \prod_{i \in S, i \neq j} p_i})^{p_j} $. So there must be some $p_j \in S such that g^{\frac{p-1}{p_j}} =g^{q \prod_{i \in S, i \neq j} p_i} =1$.
    \end{mysolution}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  \begin{parts}
    ...
    $k$ to the adversary.
		...
		Yes, this is certainly possible. Suppose we have a key of length 100, and a message of length 1. Then we can pad the last 99 bits of the plaintext before XORing it with the key, thereby revealing the last 99 digits of the secret key. However, no information 
		\end{mysolution}
